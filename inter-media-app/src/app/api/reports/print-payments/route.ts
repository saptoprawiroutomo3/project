...
export async function GET(request: NextRequest) {
  try {
    ...
    const successful = orders.filter(order => ['paid', 'processed', 'shipped', 'done'].includes(order.status)).length;
        ...
        <tbody>
          ${orders.slice(0, 50).map((order, index) => `
            <tr>
              ...
              <td>${['paid', 'processed', 'shipped', 'done'].includes(order.status) ? 'Lunas' : order.status === 'cancelled' ? 'Gagal' : 'Pending'}</td>
    ...
    console.error('Print payments error:', error);
